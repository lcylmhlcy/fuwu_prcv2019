...
if args.data == 'cifar10':
    ...
    args.num_classes = 6   # ImageNet class: 1000
...
def main():
    ...
    test_dir = '/home/computer/LCY/data/IDADP-PRCV2019-test/fuza100'
    ...
    if args.evalmode is not None:
        ...
        if args.evalmode == 'anytime':
            model.eval()
            with torch.no_grad():
                with open('fuza.json', 'w') as json_file:
                    ...
                    for img_name in os.listdir(test_dir):
                        ...
                        img_pre = test_transform(img)
                        input = torch.unsqueeze(img_pre, 0)
                        input = input.cuda()
                        ...
                        pred = int(output.argmax(dim=1, keepdim=True)) + 1

                        pred_write = {"image_id":img_name, "disease_class":pred}
                        jsfile.append(pred_write)
                        # json.dumps(pred_write, json_file)
                        
                        index += 1
                        print(index, img_name, pred)

                    json_file.write(str(jsfile))

        else:
            # the whole data given is val_data.
            # the test data is test_data.
...
def main_bak():
    ...
    model = getattr(models, args.arch)(args)
    n_flops, n_params = measure_model(model, 200, 300)    # measure_model(model, H, W) model, IM_SIZE, IM_SIZE
